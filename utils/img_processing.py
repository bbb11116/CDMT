...
def save_image_batch_to_disk(tensor, circle_list, output_dir, file_names,
                             img_shape=None, arg=None, is_inchannel=False, circle= True ):
            ...
            if arg.circle is True:
                ...
                for detections in final_results_circle:
                    ...
                    for det in detections:
                        ...
                        # 绘制圆
                        cv2.circle(fuse, (x, y), r, (0, 255, 0), 2)
...
def restore_rgb(config, I, restore_rgb=False):
    ...
    :param I: and image or a set of images  图像信息
    :return: an image or a set of images restored 返回图像信息
    """

    if len(I) > 3 and not type(I) == np.ndarray:
        I = np.array(I)
        I = I[:, :, :, 0:3]   # 保留前三个通道RGB的信息
        n = I.shape[0]
        for i in range(n):
            x = I[i, ...]  # 会的当前对应通道的数据
            x = np.array(x, dtype=np.float32)   # 将x转换为np.float32类型的numpy数组
            x += config[1]          # ？？？
            if restore_rgb:
                x = x[:, :, config[0]]
            x = image_normalization(x)     # 图像归一化
            I[i, :, :, :] = x
...
def visualize_result(imgs_list, arg):
    ...
    :param imgs_list: a list of prediction, gt and input data   预测数据，gt，输入数据
    :param arg:
    :return: one image with the whole of imgs_list data    输出一个包含所有预测数据的可视化图像
    """
